...
public class PIDSimMTL {
	...
	public static void main(String[] args) {
		...
		try{
			...
			File[] csvFiles = Utility.getFiles(new File(workingDir),".txt");int numGroups = csvFiles.length;
			for(int g = 0; g < numGroups; g++){//now loop thru files
				...
				System.out.println("Done Stage 1 of PIDSimMTL");
				
				PrintWriter out = new PrintWriter("Results\\PIDSimMTL_"+groupName+".csv");
				...
				out.close();
				System.out.println("Finished PIDSimMTL Group: "+groupName);
				System.out.println("----------------------------");
